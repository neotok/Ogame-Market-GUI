...
function getTaux() {
    ...
    if (localTaux) {
        let splited = Array.from(localTaux.split(':')).map(item => Number(item));
        return splited;
    }
    else {
        return tauxArray;
...
function initSuperMarket() {
    ...
    items.forEach((item) => {
        ...
        var itemPriceElement = item.querySelector('.details .quantity');
        var itemPriceStr = parseInt(itemPriceElement.textContent.replace(/\./g, ''));
        var wantedTypeElement = item.querySelector('.price h3');
        var wantedTypeStr = wantedTypeElement.textContent;
        var wantedPrice = item.querySelector('.price .quantity');
        var wantedPriceStr = parseInt(wantedPrice.textContent.replace(/\./g, ''));
        ...
        if (ressType.includes(itemTypeStr)) {
            let taux = parseFloat((itemPriceStr / wantedPriceStr).toFixed(2));
        ...
        if (shipArray.includes(itemTypeStr)) {
            ...
            var ship_buyable = (wantedPriceStr / localTaux * tauxArray[0] / usm_real);
